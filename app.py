...
"""
    ...
    prompt_sistema = """Você é um analista sociológico operando o referencial \
...
mobilizando o aparato conceitual da pesquisa.
...
6. Usar tom DIRETO mas RIGOROSO.
...
- NÃO escreva em primeira pessoa do singular ("eu observo", "concluo", "minha análise").
- NÃO se identifique como Filipe Severo, nem se apresente como autor humano.
- Use construções impessoais ou em terceira pessoa: "a evidência sugere", "a análise
  estrutural revela", "configura-se um caso de", "observa-se que".
...
def renderizar_dossie_completo(dossie: dict, do_cache: bool, cliente_db) -> None:
    ...
    sin_cols[2].metric(
        "% desc. com crédito de equipe",
        f"{sintomas.get('pct_descricoes_com_equipe', 0):.0f}%",
        help=(
            "Heurística textual: proporção de descrições que mencionam termos "
            "associados a equipe (edição, câmera, produção, roteiro). "
            "Lista de termos não exaustiva — trate como indicador, não como medida precisa."
        ),
    )
...
def renderizar_aba_comentarios(cliente_db) -> None:
    ...
    busca_texto = st.text_input("🔎 Buscar por canal ou vídeo", "", key="filtro_voz")
    ...
    if linhas:
        ...
        if sel != "(escolher)":
            ...
            if analise:
                st.success("✓ Carregando análise canônica do corpus")
                renderizar_voz_completa(analise, comentarios=None, do_cache=True)
