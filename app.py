...
"""
    ...
    prompt_sistema = f"""Você é Filipe Severo, autor da dissertação 'O Novo "You" do YouTube' \
(PUCRS/FAMECOS, 2026), aplicando agora análise QUALITATIVA aos comentários públicos \
...
2. CALCULAR o "Índice de Pressão Produtiva" — percentual de comentários
   classificados como 'publico_patrao' (sua tese central sobre o público-gestor).
...
{{
  ...
  "sintese_qualitativa": "<3-5 parágrafos analíticos em PROSA, sem markdown nem bullets, \
conectando achados à tese da dissertação sobre 'broadcast to you', plataformização \
e captura do trabalho criativo. Use tom Severo: rigoroso, direto, politicamente \
consciente. Identifique padrões dominantes, vozes minoritárias relevantes e tensões.>",
  "contradicao_estrutural": "<se houver dossiê do canal: 1-2 parágrafos identificando \
o gap entre o que canal entrega e o que público pede; se não houver dossiê: \
string vazia ''>"
}}
...
def renderizar_voz_completa(analise: dict, comentarios: list[dict] | None, do_cache: bool) -> None:
    ...
    # =========================================================================
    # ÍNDICE DE PRESSÃO PRODUTIVA — métrica-chave
    # =========================================================================
    st.markdown("---")
    indice = analise["indice_pressao_produtiva"]

    if indice >= 30:
        cor_indice = "#FF1493"
        label_indice = "⚠️ ALTA pressão produtiva"
        interpretacao = (
            "O público está **operando como gestor**, cobrando ritmo, exigindo "
            "produtividade, comparando com outros criadores. Sintoma clássico do "
            "*'broadcast to you'* — o criador como funcionário do próprio fandom."
        )
    elif indice >= 15:
        cor_indice = "#FFD700"
        label_indice = "🟡 Pressão produtiva moderada"
        interpretacao = (
            "Há cobrança visível de produtividade, mas ainda não é dominante. "
            "Misto de espectador e gestor."
        )
    else:
        cor_indice = "#39FF14"
        label_indice = "✅ Baixa pressão produtiva"
        interpretacao = (
            "O público predominantemente NÃO opera como gestor. Pode indicar "
            "vídeo recente, tema sem audiência cativa, ou relação parassocial "
            "menos cobrativa."
        )
    ...
    if comentarios:
        ...
        filtro_sel = st.selectbox("Filtrar por dimensão", opcoes_filtro, key="filtro_voz")
...
def renderizar_voz_da_base() -> None:
    ...
    cache = buscar_cache_comentarios(cliente_db, video_id, DIAS_VALIDADE_VOZ)
    if cache:
        renderizar_voz_completa(cache, comentarios=None, do_cache=True)
        return

    # Sem cache: pipeline completo
    st.session_state["voz_feitas"] += 1
    ...
    try:
        ...
        analise_completa = buscar_cache_comentarios(cliente_db, video_id, dias_validade=1)
        if analise_completa:
...
if modulo.startswith("🔍"):
    ...
    st.info("Use os 5 módulos disponíveis na sidebar.")
